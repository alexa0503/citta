    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card card-default">
                <div class="card-body">
                    <h4 class="card-title">专属福利 - 编辑</h4>
                    <form class="needs-validation post-form" novalidate method="POST"
                        action="{{ route('cms.settings.update',$item->id) }}">
                        @csrf
                        @method("PUT")
                        @include('cms::partials.form-group',['form_name'=>'name','label_name'=>'模块名（英文，唯一）','value'=>$item->name])
                        @include('cms::partials.form-group',['form_name'=>'title','label_name'=>'标题','value'=>$item->title,'multiple_locales'=>true])
                        @include('cms::partials.images',['form_name'=>'image','label_name'=>'头部图片','images'=>$item->image])
                        @include('cms::partials.form-group',['form_name'=>'link_type','label_name'=>'链接类型','value'=>$item->link_type])
                        @include('cms::partials.form-group',['form_name'=>'link','label_name'=>'链接地址','value'=>$item->link])
                        ...
                        @include("cms::partials.form-group-textarea",['value'=>$item->descr,'form_name'=>'descr','label_name'=>'描述','multiple_locales'=>true])
                        @include("cms::partials.form-group-textarea",['value'=>$item->body,'form_name'=>'body','label_name'=>'内容','multiple_locales'=>true])
                        {{-- @include("cms::partials.form-description",['value'=>$item->body,'form_name'=>'body','label_name'=>'内容']) --}}
